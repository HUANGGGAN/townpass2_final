...
/**
 * CCTV 種子資料（使用 Place model）
 * 從 CSV 檔案讀取真的 CCTV 資料
 */
const seedPlaces = async () => {
  ...
  if (!fs.existsSync(csvPath)) {
    ...
    console.log('⚠️  Skipping CCTV seed (CSV file not found)');
    return;
  }
      ...
      if (uuid && owner && latStr && lngStr) {
        ...
        if (!isNaN(lat) && !isNaN(lng) && lat >= -90 && lat <= 90 && lng >= -180 && lng <= 180) {
          cctvList.push({
            ...
            latitude: lat,
            longitude: lng,
            name: `CCTV-${uuid.substring(0, 8)}`, // 使用 UUID 前 8 位作為名稱
            description: `監視器 - ${owner}`,
          });
        } else {
  ...
  console.log(`Seeded CCTV to Place: ${created} created, ${updated} updated, ${skipped} skipped`);
